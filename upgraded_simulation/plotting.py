...
class PlotGenerator:
    ...
    def plot_temperature_sweep(self, temperature_results):
        """Plots Young's Modulus vs Temperature to show viscoelastic effects (Mahieux-Reifsnider model)"""
        fig, ax = plt.subplots(figsize=(6, 4.5))

        temperatures = temperature_results['temperatures']
        results = temperature_results['results']

        for filler, moduli in results.items():
            color = self.filler_colors.get(filler, self.rose_pine['love'])
            marker = self.filler_markers.get(filler, 'o')

            ax.plot(temperatures, moduli, color=color, linewidth=2,
                   marker=marker, markersize=5, markevery=3,
                   markerfacecolor='white', markeredgecolor=color,
                   label=self.filler_short.get(filler, filler))

        # Highlight Glass Transition Temperature (Tg)
        Tg = 143.0
        ax.axvline(Tg, color='gray', linestyle='--', linewidth=1.5, zorder=1)
        ax.text(Tg + 5, ax.get_ylim()[1]*0.9, r'$T_g$ (143°C)', color='gray', fontsize=10)
        ax.axvspan(Tg - 10, Tg + 10, color='gray', alpha=0.1, zorder=0)

        ax.set_xlabel('Temperature (°C)')
        ax.set_ylabel("Young's Modulus (GPa) at 15 vol%")
        ax.set_xlim(20, 250)
        self._apply_academic_axes(ax)
    ...
    def create_all_plots(self, volume_fractions, mech_results, therm_results,
                         ...
                         mech_hybrid_results=None, therm_hybrid_results=None,
                         temperature_sweep_results=None):
        """
        ...
        if temperature_sweep_results:
            print("    Creating temperature sweep plot...")
            figures['temperature_sweep'] = self.plot_temperature_sweep(temperature_sweep_results)
