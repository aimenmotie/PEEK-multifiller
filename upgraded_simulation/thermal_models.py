...
# Load the C++ percolation library
try:
    _lib_path = os.path.join(os.path.dirname(__file__), 'libpercolation.so')
    _percolation_lib = ctypes.CDLL(_lib_path)
    _percolation_lib.percolation_monte_carlo.argtypes = [
        ctypes.POINTER(ctypes.c_double), ctypes.c_int,  # v_f array, n_vf
        ctypes.c_double, ctypes.c_double, ctypes.c_double, # k_m, k_f_mean, k_f_std
        ctypes.c_double, ctypes.c_double, # phi_c, t
        ctypes.c_int, # n_samples
        ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double) # out_mean, out_lower, out_upper
    ]
    _percolation_lib.percolation_monte_carlo.restype = None
    PERCOLATION_LIB_AVAILABLE = True
except Exception as e:
    print(f"Warning: Could not load C++ percolation library. {e}")
    PERCOLATION_LIB_AVAILABLE = False
...
class ThermalSimulator:
    ...
    def sweep_volume_fraction_monte_carlo(self,
        ...
        Includes McLachlan equation percolation models using the C++ backend.
        """
        results = {}
        ...
        for i, filler_name in enumerate(self.fillers.keys()):
            ...
            # Apply Percolation model for Carbon-based fillers using C++ engine
            if PERCOLATION_LIB_AVAILABLE and ('Carbon' in filler_name or 'Graphene' in filler_name):
                # Percolation threshold roughly depends on aspect ratio
                # Ref: DOI: 10.1063/1.338274 (McLachlan equation)
                ar = self.fillers[filler_name]['aspect_ratio']
                phi_c = 0.5 / ar if ar > 10 else 0.16 # Example empirical relation
                t = 2.0 # Universal critical exponent for 3D percolation

                k_f_nominal = self.fillers[filler_name]['thermal_conductivity']
                k_f_std = k_f_nominal * 0.1 # Assumed 10% variation

                n_vf = len(volume_fractions)
                out_mean = (ctypes.c_double * n_vf)()
                out_lower = (ctypes.c_double * n_vf)()
                out_upper = (ctypes.c_double * n_vf)()
                vf_c = (ctypes.c_double * n_vf)(*volume_fractions)

                _percolation_lib.percolation_monte_carlo(
                    vf_c, n_vf,
                    self.k_m, k_f_nominal, k_f_std,
                    phi_c, t,
                    n_samples,
                    out_mean, out_lower, out_upper
                )

                mc_result['mean'] = np.array(out_mean)
                mc_result['ci_lower'] = np.array(out_lower)
                mc_result['ci_upper'] = np.array(out_upper)
            ...
            results[f'{filler_name}_all_models'] = {
                ...
                'Nan': mc_result['mean'],  # Use mean from Monte Carlo (now with percolation if applicable)
