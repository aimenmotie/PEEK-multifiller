...
class MechanicalSimulator:
    ...
    def __init__(self,
        ...
        # Apply temperature dependence to matrix properties (Mahieux-Reifsnider Weibull model)
        # Reference (DOI): https://doi.org/10.1016/S0032-3861(01)00134-4
        # "Property modeling across transition temperatures in polymers: a robust macroscopic approach"
        T_g = 143.0 # Tg of PEEK in Celsius
        m = 10.0 # Weibull modulus for transition
        E_glassy = matrix_properties['young_modulus']
        E_rubbery = E_glassy * 0.1 # Modulus drops significantly above Tg

        # Weibull transition function
        if temperature < T_g:
            self.E_m = E_glassy
        else:
            # Simplistic adaptation of the MR model for demonstration
            self.E_m = E_rubbery + (E_glassy - E_rubbery) * np.exp(-np.power((temperature - T_g)/T_g, m))
