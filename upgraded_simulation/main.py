...
class PEEKCompositeSimulation:
    ...
    def __init__(self, random_seed=42):
        ...
        self.temperature_sweep_results = None
        self.figures = {}
    ...
    def run_temperature_sweep(self):
        """
        Sweeps through temperatures to calculate viscoelastic modulus change.
        """
        print("\n" + "-" * 30)
        print("TEMPERATURE SWEEP (Viscoelastic Effect)")
        print("-" * 30)

        temperatures = np.linspace(20, 250, 20) # 20°C to 250°C
        vol_fraction = 0.15 # Fixed 15% volume fraction for this study

        temp_results = {}
        for filler in self.filler_properties.keys():
            moduli = []
            for T in temperatures:
                # Reinstantiate mechanical simulator with specific temperature
                mech_sim_T = MechanicalSimulator(
                    self.matrix_properties,
                    self.filler_properties,
                    random_state=self.random_seed,
                    temperature=T
                ...
                # Use deterministic for speed in temperature sweep
                E_f = self.filler_properties[filler]['young_modulus']
                ar = self.filler_properties[filler]['aspect_ratio']
                E_comp = mech_sim_T.halpin_tsai(vol_fraction, E_f, ar)
                moduli.append(E_comp)

            temp_results[filler] = np.array(moduli)

        self.temperature_sweep_results = {
